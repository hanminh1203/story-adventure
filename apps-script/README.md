...
- **In-cell image not showing** — make sure it was added with **Insert → Image → Image in cell** (not a floating image over the grid), and that the Web App owner can access the Sheet. The matching URL column (`avatarUrl` / `collectibleUrl` / `imageUrl`) caches the image's URL; clear that cell to force a re-resolve if the link went stale.
